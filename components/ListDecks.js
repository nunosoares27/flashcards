          ...
          <List
            ...
            renderRow={item => (
              <ListItem
                ...
                onPress={() => this.props.history.push(`/deck/${item.title}`)}
              >
                <Text
                  style={{ fontSize: 20, fontWeight: "bold", color: "white" }}
                >
                  {item.title} ({item.questions.length} Decks)
                </Text>
              </ListItem>
            )}
          />
...
export default (ListDeckswithRouter = withRouter(
  connect(mapStateToProps, mapDispatchToProps)(ListDecks)
));
